...
struct Individual
{
    ...
    double p11[2]; // proportion z1 offspring by z1 senior
    double p21[2]; // proportion z1 offspring by z2 senior
    double p10[2]; // proportion z1 offspring by z1 junior
    double p20[2]; // proportion z1 offspring by z2 junior
...
{ 
    ...
    for (size_t i = 0; i < Npatch; ++i)
    {
        ...
        for (int allele = 0; allele < 2; ++allele)
        {
            ...
            MetaPop[i].localSen.p11[allele] = h0;
            MetaPop[i].localSen.p21[allele] = h0;
            MetaPop[i].localSen.p10[allele] = h0;
            MetaPop[i].localSen.p20[allele] = h0;
            
            MetaPop[i].localJun.d[allele] = d0;
            MetaPop[i].localJun.p11[allele] = h0;
            MetaPop[i].localJun.p21[allele] = h0;
            MetaPop[i].localJun.p10[allele] = h0;
            MetaPop[i].localJun.p20[allele] = h0;
        }
...
void create_kid(Individual &mother, Individual &Kid)
{
    ...
    if (mother.z == 0)
    {
        expr_h = mother.is_senior ? 0.5 * (mother.p11[0] + mother.p11[1]) 
                                    :
                                    0.5 * (mother.p10[0] + mother.p10[1]);
    }
    else
    {
        expr_h = mother.is_senior ? 0.5 * (mother.p21[0] + mother.p21[1]) 
                                    :
                                    0.5 * (mother.p20[0] + mother.p20[1]);
    }
    ...
    for (int allele_i = 0; allele_i < 2; ++allele_i)
    {
        ...
        Kid.p11[allele_i] = mut_h(mother.p11[allele_i]);
        Kid.p21[allele_i] = mut_h(mother.p21[allele_i]);
        Kid.p10[allele_i] = mut_h(mother.p10[allele_i]);
...
void write_data()
{
    ...
    double p10,p11,p20,p21,d,vard,varp10,varp11,varp20,varp21;
    double meanp11 = 0;
    double meanp10 = 0;
    double meanp20 = 0;
    double meanp21 = 0;
    ...
    for (size_t i = 0; i < Npatch; ++i)
    {
        ...
        p10 = 0.5 * (MetaPop[i].localJun.p10[0] + MetaPop[i].localJun.p10[1]);
        p11 = 0.5 * (MetaPop[i].localJun.p11[0] + MetaPop[i].localJun.p11[1]);
        p20 = 0.5 * (MetaPop[i].localJun.p20[0] + MetaPop[i].localJun.p20[1]);
        p21 = 0.5 * (MetaPop[i].localJun.p21[0] + MetaPop[i].localJun.p21[1]);
        ...
        p10 = 0.5 * (MetaPop[i].localSen.p10[0] + MetaPop[i].localSen.p10[1]);
        p11 = 0.5 * (MetaPop[i].localSen.p11[0] + MetaPop[i].localSen.p11[1]);
        p20 = 0.5 * (MetaPop[i].localSen.p20[0] + MetaPop[i].localSen.p20[1]);
        p21 = 0.5 * (MetaPop[i].localSen.p21[0] + MetaPop[i].localSen.p21[1]);
    ...
    varp10 = ssp10 / (Npatch * 2) - meanp10*meanp10;
    varp11 = ssp11 / (Npatch * 2) - meanp11*meanp11;
    varp20 = ssp20 / (Npatch * 2) - meanp20*meanp20;
    varp21 = ssp21 / (Npatch * 2) - meanp21*meanp21;

    DataFile 
        << setprecision(5) << meanp10 << ";"
        << setprecision(5) << meanp11 << ";"
        << setprecision(5) << meanp20 << ";"
        << setprecision(5) << meanp21 << ";"
        << setprecision(5) << varp10 << ";"
        << setprecision(5) << varp11 << ";"
        << setprecision(5) << varp20 << ";"
        << setprecision(5) << varp21 << ";"
...
void write_data_headers()
{
    ...
    DataFile << "meanp10;meanp11;meanp20;meanp21;varp10;varp11;varp20;varp21;meand;vard" << endl;
}
