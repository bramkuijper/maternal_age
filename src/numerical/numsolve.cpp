...
int main (int argc, char **argv)
{
    ...
    gsl_vector *x_selgrad = gsl_vector_alloc(5);
    ...
    gsl_vector *p20_range = gsl_vector_alloc(10);
    ...
    for (iter = 0; iter < max_iter ; ++iter)
    {
        ...
        bool condition_p21 = (fabs(paramstruct.p21 - gsl_vector_get(x_selgrad, 1)) < 1e-7 || paramstruct.p21 >= 0.999) || paramstruct.p21 <= 0.001;
        bool condition_p10 = (fabs(paramstruct.p10 - gsl_vector_get(x_selgrad, 2)) < 1e-7 || paramstruct.p10 >= 0.999) || paramstruct.p10 <= 0.001;
        bool condition_p20 = (fabs(paramstruct.p20 - gsl_vector_get(x_selgrad, 3)) < 1e-7 || paramstruct.p20 >= 0.999) || paramstruct.p20 <= 0.001;

        if (condition_p10 && condition_p20 && condition_p11 && condition_p21 && (
            fabs(paramstruct.mt - gsl_vector_get(x_selgrad, 4)) < 1e-07))
        {
            ...
            paramstruct.p10 = gsl_vector_get(x_selgrad, 2);
            paramstruct.p20 = gsl_vector_get(x_selgrad, 3);
...
//                cout << "previous t - " << ik << " is: " << paramstruct.p1 << " current p1: " << paramstruct.p1 << " compare: " << fabs(gsl_vector_get(p1_range, ik)-paramstruct.p1) << endl;
 //               cout << "previous t - " << ik << " is: " << paramstruct.p2 << " current p2: " << paramstruct.p2 << " compare: " << fabs(gsl_vector_get(p2_range, ik)-paramstruct.p2) << endl;
                ...
                if (!found_in_range3 && fabs(gsl_vector_get(p10_range, ik)-paramstruct.p10) < 1e-10)
                {
                    found_in_range3 = true;
                }
                
                if (!found_in_range4 && fabs(gsl_vector_get(p20_range, ik)-paramstruct.p20) < 1e-10)
                {
        ...
        for (int ik = 9; ik > 0; --ik)
        {
            ...
            gsl_vector_set(p20_range, ik, gsl_vector_get(p20_range, ik - 1));
        ...
        gsl_vector_set(p20_range, 0, paramstruct.p20);
